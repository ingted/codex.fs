...
- artifact manifest。
- redacted run note / summary。
- ready-to-ack persistence boundary。

若 engine 支援 event stream，例如 Codex CLI `--json`，必須保存 JSONL event stream。

Raw prompt/stdout/stderr/final artifacts 預設是 private host artifact，不得自動寫入 public repo。公開匯出只能使用 redacted note/summary/reference，且需通過敏感字串掃描。
