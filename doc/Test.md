...
| T-CLI-002 | CLI-002 | TC-CLI-002 CLI send through MessageFabric | Integration | Real host + real MessageFabric | CLI-001/PTCS-002 | `dotnet run --project .\tests\codex.fs.Tests\codex.fs.Tests.fsproj --no-restore` passed; CLI HTTP client posted to advertised non-loopback host URI, host returned 202, and real PTCS inbox contained the prompt for the derived session participant | Pass | None | SD §14 |
| T-CLI-003 | CLI-003 | TC-CLI-003 attach/drain/status | Integration/CLI | Real host + real MessageFabric | CLI-002 | attach/drain/status transcript artifact | Planned | None | SD §14 |
