...
| T-HOST-006 | HOST-006 | TC-HOST-006 `/chat` form send through MessageFabric | Integration/Docs/UI | Real HTTP host endpoint plus real PTCS MessageFabric; no fake mailbox | HOST-004/CLI-007 | `dotnet run --project .\tests\codex.fs.Tests\codex.fs.Tests.fsproj --no-restore` passed; test asserted `GET /chat` returns HTTP 200 HTML form, `POST /chat` accepts a prompt, response shows `Accepted`, health exposes `chatUri`, OpenAPI includes `/chat`, and session status contains the submitted prompt | Pass | None | SD §9, §10 |
