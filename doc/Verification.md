# Verification

## Purpose

本文件記錄 codex.fs 的 real-path verifier。新增 verifier 前需先在此說明缺口、用途、適用範圍、是否 mutating、是否讀取 secret path，以及對應 Test/WBS。

## Verifiers

| Verifier | WBS/Test | Purpose | Scope | Mutating | Secret handling | Revision |
| --- | --- | --- | --- | --- | --- | --- |
| `misc/verifyMessageToEngineReply.fsx` | `E2E-002` / `T-E2E-002` | 驗證 participant -> real PTCS MessageFabric -> host single-cycle runner -> real Agy engine -> artifacts -> PTCS reply。 | Local workstation with built Debug assemblies and installed `agy` headless CLI. | Yes: writes `.codex.fs/e2e002-artifacts/` and sends in-process PTCS messages. | Does not read secret files or print secret values; invokes the installed CLI using the current user's existing CLI auth/session. | `rev-20260704-001` |
| `misc/verifyActorRuntimeArtifactProvider.fsx` | `ACTOR-003` / `T-ACTOR-003` | 驗證 WorkerActor 透過 real PTCS ActorFabric/MessageFabric 呼叫 shared runtime cycle，執行 installed Agy，產生 artifacts 並回覆 refs。 | Local workstation with built Debug assemblies and installed `agy` headless CLI. | Yes: writes `.codex.fs/actor003-artifacts/` and sends in-process PTCS messages. | Does not read secret files or print secret values; invokes the installed CLI using the current user's existing CLI auth/session. | `rev-20260705-001` |
| `misc/verifyArtifactRefsInPtcsShell.fsx` | `WEBR-007` / `T-WEBR-007` | 驗證 real ACTOR-003 artifact refs 進入 real PTCS classic `/chat` 後被 WebSharper bundle render 成 artifact card。 | Local workstation with built Debug assemblies, installed `agy`, LAN-accessible host address and Playwright-compatible Chromium/Chrome. | Yes: writes `.codex.fs/actor003-artifacts/`, `.codex.fs/webr007-pcsl/`, `.playwright-mcp/webr007/`, starts bounded `codex.fs.host` webshell and sends PTCS chat messages. | Does not read secret files or print secret values; invokes installed CLI using current user's auth/session; MessageFabric body contains redacted summary and refs only. | `rev-20260705-001` |
| `misc/verifyPtcsAiChatE2E.fsx` | `E2E-004` / `T-E2E-004` | 驗證真人視角 browser prompt 透過 PTCS classic `/chat` composer 送到 Foreman，Foreman ActorFabric worker 消費 MessageFabric、執行 installed Agy、保存 artifacts 並回覆 artifact card。 | Local workstation with built Debug assemblies, installed `agy`, LAN-accessible host address and Playwright-compatible Chromium/Chrome. | Yes: writes `.codex.fs/e2e004-artifacts/`, `.codex.fs/e2e004-pcsl/`, `.playwright-mcp/e2e004/`, starts bounded `codex.fs.host` webshell and sends PTCS chat messages through browser UI. | Does not read secret files or print secret values; invokes installed CLI using current user's auth/session; public reply contains redacted summary and refs only. | `rev-20260705-001` |
| `misc/verifyAiIntentOutputProjection.fsx` | `WEBR-010` / `WEBR-011` | 驗證 AI Chat append page 投影 Foreman reply，並開啟 floating stdio panel 透過 same-origin artifact handler 讀取 current-run final/stdout refs。 | Local workstation with built Debug assemblies, installed `codex`, LAN-accessible host address and Playwright-compatible Chromium/Chrome. | Yes: writes `.codex.fs/webr010-artifacts/`, `.codex.fs/webr010-pcsl/`, `.playwright-mcp/webr010/`, starts bounded `codex.fs.host` webshell unless using existing host URL. | Does not print secret values; artifact handler only accepts relative paths under configured artifact root and caps response bytes. | `rev-20260708-001` |

## Notes

- `misc/verifyMessageToEngineReply.fsx` is a real E2E verifier, not a fake/mock smoke path.
- The verifier may consume the user's existing CLI subscription/API allowance because it runs the installed engine.
- `.codex.fs/` is ignored by Git and stores local verifier artifacts only.

## Planned Verifier Specs

| Planned verifier | WBS/Test | Purpose | Status |
| --- | --- | --- | --- |
| `TC-UI-001A..E` future PTCS browser + MessageFabric verifier | `UI-001` / `T-UI-001` | Verify codex.fs PTCS Web UI extension manifest, real MessageFabric send/reply, artifact reference rendering, LAN/DNS advertised host status, and fallback behavior. | Specification only in `doc/RFC/RFC-UI-0001.ptcs-web-ui-extension.md`; no script exists yet and no fake/mock UI smoke is accepted. External PTCS seam references: `RFC-PTC-SPA-0006`, `RFC-PTC-SPA-0008`, `RFC-PTC-SPA-0010`, `RFC-SPA-UPSTREAM-0001`, `RFC-SPA-UPSTREAM-0002`, `RFC-SPA-UPSTREAM-0003`. |
