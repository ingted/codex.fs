...
目前狀態：`0.1.0-alpha.6`，仍是早期 alpha。此 package 目前提供：
...
- host operator landing page、PTCS chat guard、diagnostics session-send form、OpenAPI JSON 與 Swagger UI；
...
dotnet tool install codex.fs.cli --tool-path .\.codex.fs\tool --add-source .\.codex.fs\packs --version 0.1.0-alpha.6
dotnet tool install codex.fs.tool --tool-path .\.codex.fs\tool --add-source .\.codex.fs\packs --version 0.1.0-alpha.6
dotnet tool install codex.fs.host.tool --tool-path .\.codex.fs\host-tool --add-source .\.codex.fs\packs --version 0.1.0-alpha.6
...
dotnet tool install --global codex.fs.cli --add-source G:\codex.fs\bin\ptcs-hub-align-packs-20260705030317-alpha6 --version 0.1.0-alpha.6
dotnet tool install --global codex.fs.tool --add-source G:\codex.fs\bin\ptcs-hub-align-packs-20260705030317-alpha6 --version 0.1.0-alpha.6
dotnet tool install --global codex.fs.host.tool --add-source G:\codex.fs\bin\ptcs-hub-align-packs-20260705030317-alpha6 --version 0.1.0-alpha.6
...
- `http://10.28.112.93:10481/chat` legacy guard page that points operators to PTCS Web chat
- `http://10.28.112.93:10481/diagnostics/session-send`
...
codex.fs.cli session send --host http://10.28.112.93:10481 --prompt "hello foreman"
codex.fs.host --help
```

`session send` 不需要使用者先知道 session id。未指定 `--session` 時，CLI 會打到 host 的 default Foreman/SessionWorker endpoint，目標 participant 為 `<ptcs.sessionParticipantPrefix>.foreman`。指定 `--session <id>` 時，目標才是 `<ptcs.sessionParticipantPrefix>.<id>`。Use `--worker-id <participantId>` only when the prompt should go directly to a specific worker participant.

`/chat` is not a codex.fs product chat implementation. Browser chat belongs to the PTCS WebSharper chat room. The standalone host `/chat` route is a guard page; `/diagnostics/session-send` is only a diagnostic form over the standalone host's current MessageFabric. Production PTCS Web integration still requires caller-owned PTCS MessageFabric from the PTCS Host process or a peer PTCS ActorFabric node.
...
`src/codex.fs.web` is the PTCS WebSharper Bundle package for codex.fs AI chat extension work. Its current verified surface is the package/build seam, real `CommHub.useAIChat(...)` registration over PTCS extension APIs, `codex.fs.host` `web.profile=ptcs-webshell` composition that serves PTCS classic `/chat`, and WEBR-006 AI target/perspective/invocation controls that append `codex.fs.web.ai-intent.v1` JSON through PTCS page APIs. It is not yet the full product chat E2E until worker-run artifact/note refs and actor-invoked runtime execution are complete.
