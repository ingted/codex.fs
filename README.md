...
`src/codex.fs.web` is the PTCS WebSharper Bundle package for codex.fs AI chat extension work. Its current verified surface is the package/build seam and real `CommHub.useAIChat(...)` registration over PTCS extension APIs; it is not yet the full product chat E2E until host composition and ActorFabric worker visibility are complete.
